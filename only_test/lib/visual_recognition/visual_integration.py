...
class VisualIntegration:
    ...
    async def find_and_tap(self, 
        ...
        try:
            ...
            # 记录策略使用情况
            if hasattr(self.element_recognizer, 'strategy_manager') and hasattr(self.element_recognizer.strategy_manager, 'record_strategy_performance'):
                strategy = None
                if result.strategy_used != "error":
                    try:
                        strategy = RecognitionStrategy.from_string(result.strategy_used)
                    except Exception:
                        strategy = None
                if strategy:
    ...
    async def smart_recognize(self, 
        ...
        try:
            ...
            if result.strategy_used != "error":
                try:
                    current_strategy = RecognitionStrategy.from_string(result.strategy_used)
                except Exception:
                    current_strategy = None
    ...
    async def force_strategy(self, strategy: Union[str, RecognitionStrategy]) -> bool:
        ...
        try:
            if isinstance(strategy, str):
                strategy = RecognitionStrategy.from_string(strategy)
