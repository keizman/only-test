...
class ElementRecognizer:
    ...
    async def _recognize_with_xml(self, filter_package: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            # 这里调用现有的XML识别逻辑
            from ..pure_uiautomator2_extractor import EnhancedUIAutomator2Extractor
            
            extractor = EnhancedUIAutomator2Extractor()
            ...
            for element in elements:
                ...
                # Safely derive content_desc from available fields/metadata
                try:
                    _md = getattr(element, 'metadata', {}) or {}
                except Exception:
                    _md = {}
                _content_desc = getattr(element, 'content_desc', None)
                if _content_desc is None:
                    _content_desc = _md.get('content_desc') or ''

                result.append({
                    "uuid": element.uuid,
                    "text": element.text,
                    "resource_id": element.resource_id,
                    "content_desc": _content_desc,
