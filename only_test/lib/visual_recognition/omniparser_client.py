...
class OmniparserClient:
    ...
    async def analyze_screen(self, 
        ...
        try:
            ...
            # 基本有效性校验，避免向服务器发送无效/空图片
            if not self._is_valid_base64_image(screenshot_base64):
                execution_time = time.time() - start_time
                msg = "无效的截图数据(base64)，已跳过Omniparser请求"
                logger.warning(msg)
    ...
    def _is_valid_base64_image(self, b64: str) -> bool:
        """Validate base64 image string before sending to server.

        Checks:
        - Decodable base64
        - Minimum byte size threshold (avoid empty/too small frames)
        - PNG/JPEG/WebP header signature
        - Optional: PIL open/verify to ensure decodability
        """
        ...
        # Size threshold: 10KB minimal to avoid blank frames
        if len(raw) < 10 * 1024:
            return False
        ...
        is_webp = head[:4] == b"RIFF" and raw[8:12] == b"WEBP"
